...
const ChatHeader = ({ chat, currentUserId }: Props) => {
  ...
  const { typingUsers } = useIsTyping(chat);
    ...
    >
      ...
      <div>
        {typingUsers.length > 0 && isGroup && (
          <>
            <div className="text-primary">someone is typing...</div>
            {typingUsers.map((user) => (
              <span className="mr-2 text-primary" key={user.currentUserId}>
                {user.name}
              </span>
            ))}
          </>
        ...
        {typingUsers.length > 0 && !isGroup && (
          <>
            <div className="text-primary">{name} is typing...</div>
          </>
        )}
      </div>
    </div>
